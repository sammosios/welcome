...
const COMMAND_DEFS: ({ name: string } & CommandDef)[] = [
...

  {
    ...
    fn: () => [
      ...
      gap(),
      ok('01  Security'),
      out('  - Least privilege. Secrets that rotate. Small blast radius.'),
      gap(),
      ok('02  Reliability'),
      out('  - Measured by absence. SLOs, graceful degradation.'),
      gap(),
      ok('03  Observability'),
      out('  - If you cannot measure it, you cannot improve it.'),
      gap(),
      ok('04  Scalability'),
      out('  - Systems that grow with load. No rewrites.'),
      gap(),
...

  {
    ...
    fn: () => [
      ...
      gap(),
      dim('Concepts transfer. Tools are just syntax.'),
      gap(),
    ],
  },
